...
RUNTIME_IMAGE_REF="${RUNTIME_IMAGE_REF:-unknown}"
IDLE_GAP_S="${IDLE_GAP_S:-8}"

SERVER_READY_TIMEOUT_S="${SERVER_READY_TIMEOUT_S:-420}"

RUN_DIR="${RUN_DIR:-/workspace/007a_nsys_c256_only}"
...
echo "========================================"
check_server
echo "Post-profile GPU canary"
echo "========================================"
CANARY_START_NS="$(date +%s%N)"
CANARY_START_NS="$(date +%s%N)"
RUN_ID="nsys_post_canary" RUN_DIR="$RUN_DIR/post_canary" MODEL="$MODEL" PORT="$PORT" NUM_PROMPTS=1 NUM_WARMUPS=0 REQUEST_RATE=inf MAX_CONCURRENCY=1 INPUT_LEN="$INPUT_LEN" OUTPUT_LEN=16 SEED="$SEED" bash vllm/scripts/02_bench_random_baseline.sh
CANARY_END_NS="$(date +%s%N)"
printf "START_NS\t%s\nEND_NS\t%s\n" "$CANARY_START_NS" "$CANARY_END_NS" > "$RUN_DIR/post_canary_window.tsv"
CANARY_END_NS="$(date +%s%N)"
printf "START_NS\t%s\nEND_NS\t%s\n" "$CANARY_START_NS" "$CANARY_END_NS" > "$RUN_DIR/post_canary_window.tsv"
echo "POST_CANARY_COMPLETE"
stop_profile
...
echo
echo "========================================"
TRACE_INTEGRITY_STATUS="EXPORT_FAILED"
rm -f "$RUN_DIR/server_full.sqlite"
if "$NSYS_BIN" export --type sqlite --output "$RUN_DIR/server_full.sqlite" "$RUN_DIR/server_full.nsys-rep"; then
    if bash vllm/scripts/20_validate_nsys_tail.sh "$RUN_DIR"; then
        TRACE_INTEGRITY_STATUS="OK"
    else
        TRACE_INTEGRITY_STATUS="FAILED"
    fi
fi
echo "trace_integrity_status=$TRACE_INTEGRITY_STATUS"
rm -f "$RUN_DIR/server_full.sqlite"
...
rm -f "$RUN_DIR/server_full.sqlite"
if "$NSYS_BIN" export --type sqlite --output "$RUN_DIR/server_full.sqlite" "$RUN_DIR/server_full.nsys-rep"; then
    if bash vllm/scripts/20_validate_nsys_tail.sh "$RUN_DIR"; then TRACE_INTEGRITY_STATUS="OK"; else TRACE_INTEGRITY_STATUS="FAILED"; fi
fi
echo "trace_integrity_status=$TRACE_INTEGRITY_STATUS"
rm -f "$RUN_DIR/server_full.sqlite"
...
cd "$RUN_PARENT" && sha256sum "$(basename "$ARCHIVE")" \
    > "$CHECKSUM"
